...
def _parse_labels(self, line, p, o):
    if line[0] == "(":
        #Provjeravamo je li se nakon zatvorene zagrade pojavljuju dodatni elementi u stringu
        labelAfterParenthesis = line.split(")", 1)[-1]
        label = line[1:].split(")")[0]
        if (len(label) == 0 or labelAfterParenthesis.strip() != ""):
            self._flag = False
            self._line = o
            self._errm = "Invalid label after the closing parenthesis"
        else:
